...
const getNumLessonsFromLineItems = (lineItems: Stripe.LineItem[]): number => {
	...
	const numLessons = lineItems.reduce((acc, item) => {
		...
		const numDownpayments = item.description.match(downpaymentRegex)
			? 1
			: 0;
...
export default getNumLessonsFromLineItems;
